...
**Status:** recording plan, not evidence. Record a single uninterrupted, resettable
hero run on the deployed product. Keep browser chrome, session handles, and any secrets
out of frame. If Jordan is driven synthetically, display “synthetic collaborator — same
UI/service path” for the relevant shot.
...
| 0:29–0:50 | Switch to Jordan’s genuine second browser/session. Change launch capacity 18 → 14 and cite the four-day incident rotation. Return to Maya’s live page. | “Jordan is the engineering lead in a separate session. An incident rotation cuts launch capacity from eighteen to fourteen engineer-days. This is a real attributed update, not a timer or a fabricated page state.” |
...
| 2:37–2:46 | Return to the full workspace and Capability Field. Show the final live URL only if the release deployment is real and public. | “Ratiflow makes a live page—not a static API menu—the agent’s current, reviewable action space. The source, live app, and evidence links are public for judging.” |
