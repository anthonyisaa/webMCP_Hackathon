...
const { RELEASE_CONTRACT_VERSION, sha256Text, validateReleaseManifest } =
  await import(manifestModulePath) as typeof import("./manifest");
import type { GitState, ManifestValidationOptions } from "./manifest";

export const DEFAULT_MANIFEST = "evals/release/repo-document-v4/manifest.json";
export const EXECUTING_SOURCE_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");
const REQUIRED_SOURCE_FILES = [
  "evals/release/repo-document-v4/manifest.ts",
  "evals/release/repo-document-v4/validate.ts",
  DEFAULT_MANIFEST,
] as const;
...
export type GitInspection = { sourceRoot: string; state: GitState };
...
const isPendingGate = (value: unknown) => {
  ...
  return gate.status === "PENDING"
    && gate.evidenceClass === "PENDING"
    && gate.sourceCommitSha === null
    && gate.deployedUrl === null
    && gate.deploymentId === null
    && gate.capturedAtUtc === null
    && Array.isArray(gate.artifactRefs)
    && gate.artifactRefs.length === 0;
};
...
export const isAllPendingTemplate = (value: unknown) => {
  ...
  const surface = releaseIdentity.supportedNativeSurface;
  const identityPending = [
    ...
    "submissionUrl",
    "recordedAtUtc",
  ].every((key) => releaseIdentity[key] === null)
    && Array.isArray(releaseIdentity.fixtureVersions)
    && releaseIdentity.fixtureVersions.join("|") === "repo-document-v4.postmortem.v1|repo-document-v4.product-document.v1"
    && typeof surface === "object"
    && surface !== null
    && !Array.isArray(surface)
    && ["client", "clientVersion", "browser", "browserVersion"].every((key) =>
      (surface as Record<string, unknown>)[key] === null);
  const rowSections = [
    "domainEvidence",
    "browserEvidence",
    "nativeEvidence",
  ...
  const rowsPending = rowSections.every((section) =>
    Array.isArray(manifest[section]) && (manifest[section] as unknown[]).every(isPendingGate));
  const judgesPending = Array.isArray(manifest.judges) && manifest.judges.every((entry) => {
    if (!isPendingGate(entry) || typeof entry !== "object" || entry === null || Array.isArray(entry)) return false;
    const judge = entry as Record<string, unknown>;
    return judge.phase === "FINAL"
      && judge.score === null
      && judge.evaluatorId === null
      && Array.isArray(judge.citations)
      && judge.citations.length === 0
      && judge.strongestGap === null
      && judge.mustFix === null;
  });
  return manifest.releaseStatus === "PENDING"
    && identityPending
    && rowsPending
    && isPendingGate(manifest.ablation)
    && judgesPending;
};
...
const deriveAssetRoot = (manifestPath: string, fallback: string) => {
  const suffix = join("evals", "release", "repo-document-v4", "manifest.json");
...
export function runReleaseManifestCli(
  ...
  const claimsPass = typeof value === "object"
    && value !== null
    && !Array.isArray(value)
  ...
  if (claimsPass) {
    try {
      git = (dependencies.inspectGit ?? inspectSourceGit)(EXECUTING_SOURCE_ROOT);
    } catch (error) {
      writeJson(io.stderr, {
        contractVersion: RELEASE_CONTRACT_VERSION,
        status: "INVALID",
        ...
        issues: [{
          path: "$.releaseIdentity.sourceCommitSha",
          message: `could not verify source Git state: ${error instanceof Error ? error.message : String(error)}`,
        }],
      });
      return 2;
    }
  ...
  const configuredRoot = dependencies.validationOptions?.assetRoot;
  const fallbackRoot = git?.sourceRoot ?? cwd;
  const assetRoot = configuredRoot ?? deriveAssetRoot(manifestPath, fallbackRoot);
  let validation;
  try {
  ...
  return validation.releaseReady ? 0 : validation.schemaValid ? 1 : 2;
}
