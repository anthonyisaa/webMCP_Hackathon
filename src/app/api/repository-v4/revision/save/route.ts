...
import { hasExactRequestKeys, hasPublicRequestId, idempotencyKeyFrom, invalidRepositoryRequest, repositoryResponse } from "../../_response";
...
export async function POST(request: Request) {
  ...
  if (!body || !requestId || hasPublicRequestId(body)
    || !hasExactRequestKeys(body, ["expectedRevision", "title", "body"])) {
    return repositoryResponse(invalidRepositoryRequest(
      "Save revision requires exact public JSON and Idempotency-Key.",
    ));
  }
